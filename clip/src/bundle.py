...
def main():
    ...
    ap.add_argument("--dest", default=None, help="default <repo>/<slug>")
    ...
    lsrc = f"{ROOT}/lessons/{slug}"
    if os.path.isdir(lsrc):
        ldst = f"{bdir}/lesson"
        os.makedirs(ldst, exist_ok=True)
        for f in ("lesson.py", "setup.sh"):
            if os.path.exists(f"{lsrc}/{f}"):
                shutil.copy2(f"{lsrc}/{f}", f"{ldst}/{f}")
        saved.append("lesson/")
    ...
    lines = [
        ...
        f"Frozen {stamp} from `clip/intermediate/` (shared scratch — overwritten by",
