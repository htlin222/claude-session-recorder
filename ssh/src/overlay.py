...
DIST = ROOT                           # product lands at the slug-folder root
...
def panel_img(p, k, next_key=None, num=None, total=None, next_num=None):
    ...
    for i, t in enumerate(toks):
        ...
        while ext_ce < len(cmd) and cmd[ext_ce] != " ":
            ext_ce += 1
        cs, ce = t["cs"], ext_ce
        # draw one box per wrapped line the token spans, so a token that
        # straddles a line break (e.g. "-f ./id_demo") never inverts (x1<x0).
        for li, (start, txt) in enumerate(lines):
            a = max(cs, start)
            b = min(ce, start + len(txt))
            if a >= b:
                continue
            col = a - start
            col2 = b - 1 - start
