...
SCRIPT = [
    ...
    S("加上 -g 再給一個 glob 樣式，像是星號點 md，就只鎖定 Markdown 檔。"),
    R("rg -g '*.md' TODO"),
    S("這一次，只有 README 和 guide 兩份文件被掃到。"),
    S("-g 也可以反過來用驚嘆號排除檔案，彈性比固定類型更高。"),
    ...
    S("當你只想老老實實地找一個含點的字串，就用 -F 把樣式視為固定純文字。"),
    R("rg -F '3.14' src/utils/calc.py"),
    S("這樣 3.14 就只會對到字面上的 3.14，中間那個點不再是萬用字元。"),
    S("搜尋含有大量符號的程式碼或設定值時，-F 能省去一堆跳脫字元。"),
    ...
    S("加上 -v 反轉比對，rg 會列出所有不包含樣式的行。"),
    R("rg -v error src/log.txt"),
    S("所有含小寫 error 的行都被濾掉，剩下的全是沒出錯的正常紀錄。"),
    ...
    CLR(key="--hidden：把預設略過的隱藏檔也納入搜尋",
        ...
        ("secret", "要搜尋的樣式", "path")]),
    S("還記得開頭那個用點開頭的隱藏檔嗎？"),
    S("為了避免雜訊，rg 預設會跳過隱藏檔，加上 --hidden，才會把它們也一起納入。"),
    R("rg --hidden secret"),
    S("於是，連那個藏起來的隱藏檔，裡頭的內容都被一字不漏地翻了出來。"),
    S("如果連那些被忽略規則擋掉的檔案也想一起搜，還有一個更徹底的 --no-ignore 可以搭配。"),
    ...
    S("用 -C1 就能把命中行的前後各一行脈絡一起帶出來，而只想看後面或前面，則分別有 -A 和 -B。"),
    R("rg -C1 'my error' src/log.txt"),
    S("命中的 my error 上下各補了一行，這筆錯誤的來龍去脈瞬間清楚起來。"),
    S("從預設的遞迴搜尋，到這一連串好記的旗標，就是日常用 rg 最常派上用場的招式了。"),
]
