...
SCRIPT = [
    ...
    S("不管是備份重要檔案，還是整理專案，我們每天都在命令列上複製東西。"),
    S("最基本、也最常用的工具，就是 cp，也就是英文 copy 的縮寫。"),
    S("要特別注意，macOS 內建的 cp 是 BSD 版本，跟 Linux 上的 GNU cp 在細節上並不完全一樣。"),
    S("為了讓畫面上的輸出和 Linux 使用者看到的一致，這個教學一律改用 gcp，也就是 GNU coreutils 提供的版本。"),
    S("在 Linux 上你直接打 cp 就是這個行為，在 macOS 上裝好 coreutils，就會有 gcp 可以用。"),
    S("同樣的道理，這次列檔案用 gls，看時間戳用 gstat，看內容用 gcat，都是為了和 Linux 對齊。"),
    S("接下來就從看一眼範例檔案開始，把各個旗標一步一步走過一遍。"),
    ...
    R("gls -l"),
    S("裡面有幾個文字檔，一個 docs 子目錄，還有一個刻意設成 2020 年的 stamped 檔。"),
    ...
    S("指令安靜地做完，原本的 a.txt 還在，旁邊多了一份一模一樣的副本。"),
    ...
    S("我們把目的地寫成 backup/，結尾的那一條斜線表示這是一個目錄。"),
    ...
    S("兩個檔案一次就位，這種多來源、單一目錄的寫法在日常非常常用。"),
    ...
    S("新檔案連時間戳都和來源一樣，停在 2020 年，屬性被完整保留了下來。"),
    ...
    S("加上 -n 這個旗標，遇到已經存在的檔案，就會安靜地跳過不動。"),
    R("gcp -n a.txt copy.txt"),
    S("指令什麼都沒說，因為它判斷目的檔已經存在，於是就直接略過了。"),
    R("gcat copy.txt"),
    S("把內容印出來看，還是原本那一份，證明它確實沒有被覆蓋。"),
    ...
    S("加上 -v 這個旗標，就會把每一個複製動作都即時印出來。"),
    R("gcp -v notes.txt copied.txt"),
    S("畫面上出現一行箭頭，清楚標示來源被複製成了哪個新檔案。"),

    CLR(key="短旗標可以合併：-rv", hero="gcp -rv docs docs-v", toks=[
        ("gcp", "複製", "path"),
        ("-rv", "把遞迴與囉嗦兩個短旗標併在一起", "path"),
        ("docs", "來源目錄", "path"),
        ("docs-v", "目的目錄", "path")]),
    S("短旗標其實可以黏在一起寫，不必一個一個分開打。"),
    S("把遞迴和囉嗦這兩個短旗標合起來，就能一次同時生效。"),
    R("gcp -rv docs docs-v"),
    S("整個目錄被遞迴複製，而且每一個檔案的過程也都即時印了出來。"),
    ...
    S("這個封存模式會遞迴複製，又完整保留權限與時間戳等所有屬性。"),
    R("gcp -a docs archive"),
    S("連裡面每個檔案的時間戳，都和原本的來源一模一樣。"),

    CLR(key="用 tree 檢視最後的複製成果", hero="tree -L 1", toks=[
        ("tree", "以樹狀圖顯示目錄", "path"),
        ("-L", "限制顯示的層數", "star"),
        ("1", "只看最上面一層", "path")]),
    S("操作了這麼多次，最後用一張樹狀圖把整個資料夾總覽一遍來收尾。"),
