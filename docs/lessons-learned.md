...
### 終端（真實）vs 面板（預測）切場景：侦测「清屏」帧來對齊（最確定）
終端是 VHS 實錄、面板是預測時鐘合成，scene 切換時兩者會差 0.5～0.8s，
而且**不是固定方向**（有的終端快、有的慢），所以全域提前量（`PANEL_LEAD`）治不了。
真正確定的解法：**清屏在錄像裡是可偵測的視覺事件**（畫面從滿屏文字→空提示符的硬切）。
- 用 ffmpeg 把 `rsync-demo.mp4` 抽成低解析灰階，數每幀的亮像素；
  content 由高驟降到接近 baseline 的那幀 = 一次清屏（`overlay.py: detect_clears`）。
- 7 段會偵測到 6 次 content→empty 轉場，把每段面板 banner **釘在偵測到的真實幀**，
  scene 內 token 再按預測相對偏移。左右就**同一幀**切換，零殘差。
- `PANEL_LEAD` 縮成 ~1 幀（0.04s）當保險，確保面板不晚於清屏。
- 偵測數量若不等於段數則 fallback 回預測，不致整支壞掉。
